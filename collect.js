...
// マスタデータ（駅名マップ、編成マップ、セクションマップ）の定義
...
async function fetchTrainLocation() {
  try {
    let unyoData = { toyoko: {}, meguro: {}, dento: {}, lastUpdate: "" };
    ...
    // 1. 東横線 (toyoko.json + shinyokohama.json)
    ...
    // 2. 目黒線 (meguro.json + shinyokohama.json)
    ...
    for (const t of (dtRes.trains || [])) {
      ...
      const p = num[0];
      if (p === "1" && t.kind !== "回") continue; // 不定期臨時除外
